...
describe('R2 Bindings Tests', () => {
  ...
  it('should handle multiple R2 bindings', async () => {
    ...
    const tests = [
      {
        config: { binding_name: 'IMAGES_BUCKET' },
        key: 'test.jpg',
        expectedBucket: 'IMAGES_BUCKET',
      },
      {
        config: { binding_name: 'ASSETS_BUCKET' },
        key: 'test.webp',
        expectedBucket: 'ASSETS_BUCKET',
      },
    ];
  ...
  it('should return appropriate headers from R2 metadata', async () => {
    ...
    expect(response.headers.get('x-meta-content-disposition')).toBe(

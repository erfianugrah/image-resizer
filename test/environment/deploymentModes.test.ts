...
const handleImageRequest = vi.fn(async (request: Request, config: Record<string, any>, env: Record<string, any>) => {
...
async function mockWorker(request: Request, env: Record<string, any>, ctx: ExecutionContext): Promise<Response> {
...
describe('Deployment Mode Tests', () => {
  ...
  it('should use R2 objects in r2-only mode', async () => {
    ...
    expect(handleImageRequest).toHaveBeenCalledWith(mockRequest, expect.objectContaining({ mode: 'r2-only' }), mockEnv);
  });
  ...
  it('should use remote origin in remote-only mode', async () => {
    ...
    expect(handleImageRequest).toHaveBeenCalledWith(mockRequest, expect.objectContaining({ mode: 'remote-only' }), mockEnv);
  });
  ...
  it('should use R2 in hybrid mode when available', async () => {
    ...
    expect(response.headers.get('x-source')).toBe('r2-direct');
    expect(handleImageRequest).toHaveBeenCalledWith(mockRequest, expect.objectContaining({ mode: 'hybrid' }), mockEnv);
  });
  ...
  it('should use custom R2 binding name when specified', async () => {
    ...
    expect(response.headers.get('x-deployment-mode')).toBe('r2-only');
    expect(response.headers.get('x-source')).toBe('r2-direct');
    expect(handleImageRequest).toHaveBeenCalledWith(customRequest, expect.objectContaining({ 
  ...
  it('should throw if environment is missing mandatory configuration', async () => {
    ...
    await expect(mockWorker(mockRequest, mockEnv, mockCtx as any))
